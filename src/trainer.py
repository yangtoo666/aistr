...
class Trainer(object):
    ...
    def train(self):
        for epoch in range(self.start_epoch, self.epochs):
            ...
            # 锁定预训练模型为评估模式
            # if hasattr(self.model.module, 'pretrainsef'):
            #     print("Locking pretrainsef model for evaluation.")
            #     self.model.module.pretrainsef.eval()  # 强制保持评估模式
            # tr_loss = self._run_one_epoch2(epoch)  # 训练模型

            # tr_loss = self.diff_run_one_epoch(epoch)
            # tr_loss = self.helmholtz_train_one_epoch(epoch)
            tr_loss = self.physics_regularized_train_one_epoch(epoch)
            gc.collect()
            ...
            val_loss = self._run_one_epoch1(epoch, cross_valid=True)  # 验证模型
    ...
    # 在trainer.py的Trainer类中添加以下方法

    # trainer.py
    def physics_regularized_train_one_epoch(self, epoch, cross_valid=False):
        """使用真正正则化的训练方法"""
        ...
        for i, (data) in enumerate(data_loader):
            ...
            constrained_sources, weighted_physics_loss, raw_physics_loss, lambda_physics  = self.model.physics_regularizer(
                separated_sources, padded_mixture, current_data_loss,compute_physics_loss
            )
            ...
            # # ---------- 3. 正则化损失 ----------
            # if not cross_valid:
            #     # 3.1 彻底断开数据损失的图，避免正则化反向时把主图也留住
            #     data_loss_det = data_loss.detach().clone()
            #     regularization_loss = self.model.compute_regularization_loss(
            #         [s.detach() for s in separated_sources],  # 把分离结果图也断掉
            #         data_loss_det
            #     )
            # else:
            #     regularization_loss = torch.tensor(0.0, device=data_loss.device)

            # 总损失 = 数据损失 + 正则化损失
            total_batch_loss = data_loss + 0.1 * weighted_physics_loss
            regularization_loss=weighted_physics_loss
            # if not cross_valid:
            ...
            #     self.optimizer.step()
            if not cross_valid:
                self.optimizer.zero_grad(set_to_none=True)  # 更快更干净
                total_batch_loss.backward()
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.max_norm)
                self.optimizer.step()
            ...
            if i % self.print_freq == 0:
                print('Epoch {0} | Iter {1} | Total Loss {2:.3f} | Data Loss {3:.3f} | Reg Loss {4:.3f} | {5:.1f} s/batch'.format(
                    epoch + 1, i + 1, total_loss / (i + 1), data_loss.item(),
                    regularization_loss.item(), run_time / (i + 1)), flush=True)

            # ---------- 6. 显存泄漏关键：手动清图 + 清缓存 ----------
            del sep1, sep2, target1, target2, loss1, loss2, data_loss, regularization_loss, total_batch_loss
            torch.cuda.empty_cache()

            # ---------- 7. TensorBoard 只写标量，不带图 ----------
        avg_reg = total_regularization_loss / (i + 1)
        self.write.add_scalar("regularization_loss", avg_reg, epoch + 1)
        ...
        for i, (data) in enumerate(data_loader):
           ...
           # 模型前向传播
            ...
            with torch.no_grad():
                ...
                constrained_sources,_ ,_,_= self.model.physics_regularizer(
            ...
            # 计算SI-SNR损失
            target1 = padded_source[:, 0:1, :]  # 第一个说话人
            target2 = padded_source[:, 1:2, :]  # 第二个说话人
