...
class VectorizedMorletCWT(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        wavelet_fft[positive_mask] = (
                torch.pi ** (-0.25) *
                torch.sqrt(
                    2 * torch.tensor(torch.pi, dtype=torch.float32, device=device) * scales_expanded.expand_as(omega)[
                        positive_mask]) *
                gaussian[positive_mask] *
                torch.exp(1j * self.w0 * (omega[positive_mask] - self.w0))
        ).to(torch.complex64)
...
class OptimizedMorletCWTHelmholtzLoss(nn.Module):
    ...
    def update_valid_mask(self, scales, device):
        """更新有效掩码和相关缓冲区以匹配新的尺度"""
        scales_tensor = torch.tensor(scales, device=device)
        center_freqs = 5.0 * self.fs / (2 * np.pi * scales_tensor)
        valid_mask = (center_freqs >= self.freq_low) & (center_freqs <= self.freq_high)
        valid_center_freqs = center_freqs[valid_mask]
        valid_scales = scales_tensor[valid_mask]
        valid_k = 2 * np.pi * valid_center_freqs / self.c
        valid_dx = self.c / self.fs * valid_scales
        valid_dx_squared = valid_dx ** 2

        # 更新缓冲区
        self.valid_mask_buffer = torch.tensor(valid_mask, device=device)
        self.valid_center_freqs_buffer = valid_center_freqs
        self.valid_k_buffer = valid_k
        self.valid_dx_squared_buffer = valid_dx_squared
        self.valid_scales_buffer = valid_scales
    ...
    def forward(self, waveform):
        ...
        need_update = (
                abs(actual_duration - self.duration) > 0.1 or
                len(scales) != len(self.cwt.scales_buffer)
        )
